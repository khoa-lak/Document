...
bool CLASS_SIMA7xxx::Func_SerialSIM(char* AT, char* ATCheck) {
  ...
  if (SerialAT.available() > 0) {
    unsigned long previousTick = millis();
    unsigned long timeOut = 200;
    if (strstr(AT, "AT+CMQTTSUB"))
      timeOut = 5000;
    //    Serial.print(String(timeOut) + " ");
    //    Serial.println(AT);
    while (previousTick + timeOut > millis()) {
  ...
  uint16_t len = inputString.length() + 1;
  char* input = new char[len];
  inputString.toCharArray(input, len);
  delay(1000);
  if (strstr(input, ATCheck)) {
    if (strstr(AT, "AT+CMQTTSUB")) {
      Func_SplitString(input);
      memset(c_setting, 0, sizeof(c_setting));
      strcpy(c_setting, input);
      Serial.print("setting ban đầu: ");
...
bool CLASS_SIMA7xxx::Func_FollowTopic1() {
  ...
  //    done = Func_UnFollowTopicSIM(c_topicDeviceID);
  //    if (!done)
  //      Func_ResetVariable();
  //  }
  else {
...
bool CLASS_SIMA7xxx::Func_PubMesSIM(char *mesPub, char *topic) {
  if (Func_UnSubMes(topic)) {
    Serial.println("tao pub nè");
