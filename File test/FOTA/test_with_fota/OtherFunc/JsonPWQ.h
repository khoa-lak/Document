...
#define MSG_MID 511
#define MSG_MAX 2048
#define JSON_LARGE_BUFFER_SIZE 6144
class JsonPWQ {
private:
  char _sharedBufferMin[MSG_MIN];
  char _sharedBufferMid[MSG_MID];
  char _sharedBufferMax[MSG_MAX];
  void clearDynamicArrays(ParamReagent& reagent);
public:
  bool isJon(char* js);
  bool handleMsgSettingDevice(char* msg, SettingDevice stgDevice, ParamPond paramPond[NUM_POND], ParamReagent paramReagent[NUM_REAGENT], ParamSensor paramSensor[NUM_SENSOR]);
  void handleMsgSettingPond(char* msg, ParamPond paramPond[NUM_POND]);
  void handleMsgManual(char* msg, ControlManual& controlManual);
  void handleMsgMeasureNow(char* msg, ParamPond paramPond[NUM_POND], ParamReagent paramReagent[NUM_REAGENT], ParamSensor paramSensor[NUM_SENSOR], ListReagentNow& listReagentNow);
  bool handleMsgSchedule(char* msg, int8_t* data, uint32_t current_time, ParamReagent paramReagent[NUM_REAGENT], ParamSensor paramSensor[NUM_SENSOR]);

  char* msgStateMachine(char* title, uint32_t unix_time, uint16_t total_time, float temp, int8_t pond, ParamPond paramPond[NUM_POND], ParamReagent paramReagent[NUM_REAGENT], ParamSensor paramSensor[NUM_SENSOR], uint8_t param_queue[][3], uint8_t num_queue, char* test_id, char* state);
  ...
  char* msgInforReagent(const char* reagent, uint8_t pos, float ml_used);
  char* msgInforVimWater(uint8_t type, float ml_used);
  //for lora
