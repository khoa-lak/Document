...
bool SimA7xxx::downloadToFlash(uint32_t start_addr) {
  ...
  const char *http_resource = "https://upcode.iotoom.com/khoa%20test/FactorySetting.ino.hex";
...
void SimA7xxx::parseHexLine(char *line) {
  ...
  //uint32_t targetAddr = OTA_STORAGE_ADDR + (addr - 0x8000);
  if (type == 00) {  // Record Type 00: Dữ liệu (Data)
                     // Địa chỉ tuyệt đối mà file HEX muốn ghi:
    uint32_t absoluteAddrInHex = currentExtendedAddr + addr;
    // Địa chỉ bắt đầu của firmware gốc (phải khớp với file HEX, thường là 0x08000000 + DVECT_OFFSET)
    uint32_t firmwareStartAddr = 0x08000000 + 0x8000;
    // Tính toán địa chỉ ghi vào vùng OTA:
    uint32_t targetAddr = OTA_STORAGE_ADDR + (absoluteAddrInHex - firmwareStartAddr);
    ...
    currentExtendedAddr = (uint32_t)upperAddr << 16;
    Serial.printf("--- Base Address Updated to1: 0x%08X ---\n", currentExtendedAddr);
