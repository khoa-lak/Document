...
bool SimA7600::pubData(char* mes, uint8_t type) {
  char* topic = nullptr;
  uint8_t len = 0;
  bool done = false;
  if (type == 0) {
    len = strlen(paramSim.topicBH1750);
    topic = new char[len];
    strcpy(topic, paramSim.topicBH1750);
  }
  else if (type == 1) {
    len = strlen(paramSim.topicTCS34725);
    topic = new char[len];
    strcpy(topic, paramSim.topicTCS34725);
  }
  else if (type == 2) {
    len = strlen(paramSim.topicTEMT6000);
    topic = new char[len];
    strcpy(topic, paramSim.topicTEMT6000);
  }
  if (topic != nullptr && len != 0) {
    done = (this->*P_PubMes)(mes, topic);
  }
  if (topic != nullptr)
    delete[] topic;
  return done;
