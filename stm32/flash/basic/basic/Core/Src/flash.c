...
uint32_t Flash_Read_Int(uint32_t addr)
{
...
void Flash_ReadChar(char* dataOut, uint32_t addr1, uint32_t addr2)
{
	...
	if((unsigned char)Flash_Read_Int(addr1+(uint32_t)2) == 255)
	{
		check = (unsigned char)Flash_Read_Int(addr2)-48;
	}
	else
	{
		check = ((unsigned char)Flash_Read_Int(addr1)-48)*10 + (unsigned char)Flash_Read_Int(addr1+2)-48;
	}
