...
#include <stdlib.h>
#include <cstdio>
// Hàm giả định để ghi log (thay cho Serial.println)
void log_print(const char* message); // Định nghĩa trong main.c hoặc một file khác
...
void UnixStamp::handleTxtDate(char* txt, uint8_t& sec, uint8_t& mins, uint8_t& hrs, uint8_t& day, uint8_t& month, uint16_t& year) {
    char* input = new char[strlen(txt) + 1];
    strcpy(input, txt);
...
uint32_t UnixStamp::convertToUnix(char* txt) {
    if (txt == nullptr)
        return 0;
    uint8_t sec = 0, mins = 0, hrs = 0, day = 0, month = 0;
    uint16_t year = 0;
...
char* UnixStamp::convertToDay(uint32_t epoch) {
    ...
    char* txt = new char[strlen(txtDate) + 1];
    strcpy(txt, txtDate);
    return txt;
}